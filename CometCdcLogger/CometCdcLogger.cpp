...
static const char* mylogger_spec[] = {
    "implementation_id", "CometCdcLogger",
    "type_name",         "CometCdcLogger",
...
CometCdcLogger::CometCdcLogger(RTC::Manager* manager)
    : DAQMW::DaqComponentBase(manager),
      m_InPort("cometcdclogger_in", m_in_data),
      ...
      m_debug(false)
{
    ...
    registerInPort("cometcdclogger_in", m_InPort);

    init_command_port();
    init_state_table();
    set_comp_name("CometCdcLogger");
}

CometCdcLogger::~CometCdcLogger()
{
}

RTC::ReturnCode_t CometCdcLogger::onInitialize()
{
    if (m_debug) {
        std::cerr << "CometCdcLogger::onInitialize()" << std::endl;
    }

    return RTC::RTC_OK;
}

RTC::ReturnCode_t CometCdcLogger::onExecute(RTC::UniqueId ec_id)
{
    daq_do();
...
int CometCdcLogger::daq_dummy()
{
    return 0;
}

int CometCdcLogger::daq_configure()
{
...
int CometCdcLogger::parse_params(::NVList* list)
{
    int ret = 0;
    ...
    for (int i = 0; i < length; i += 2) {
        ...
        if (sname == "isLogging") {
            ...
            if (svalue == "yes") {
                ...
                isExistParamLogging = true;
                std::cerr << "CometCdcLogger: Save to file: true\n";
            }
            else if (svalue == "no") {
                m_isDataLogging = false;
                isExistParamLogging = true;
                std::cerr << "CometCdcLogger: Save to file: false\n";
            }
        }
...
int CometCdcLogger::daq_unconfigure()
{
    std::cerr << "*** CometCdcLogger::unconfigure" << std::endl;
    if (m_isDataLogging) {
        delete fileUtils;
        if (m_debug) {
...
int CometCdcLogger::daq_start()
{
    std::cerr << "*** CometCdcLogger::start" << std::endl;
    ...
    if (m_isDataLogging) {
        ...
        if (ret < 0) {
            std::cerr << "### ERROR: CometCdcLogger: open file failed\n";
            fatal_error_report(CANNOT_OPEN_FILE);
        }
        else {
            std::cerr << "*** CometCdcLogger: file open succeed\n";
            m_filesOpened = true;
        }
    }
    return 0;
...
int CometCdcLogger::daq_stop()
{
    std::cerr << "*** CometCdcLogger::stop" << std::endl;

    if (m_isDataLogging && m_filesOpened) {
        if (m_debug) {
            std::cerr << "CometCdcLogger::stop: close files \n";
        }
...
int CometCdcLogger::daq_pause()
{
    std::cerr << "*** CometCdcLogger::pause" << std::endl;
    return 0;
}

int CometCdcLogger::daq_resume()
{
    std::cerr << "*** CometCdcLogger::resume" << std::endl;
    return 0;
}

int CometCdcLogger::reset_InPort()
{
    bool ret = true;
    while (ret) {
        ret = m_InPort.read();
    ...
    if (m_debug) {
        std::cerr << "*** CometCdcLogger::InPort flushed\n";
    }
    return 0;
}

void CometCdcLogger::toLower(std::basic_string<char>& s)
{
...
int CometCdcLogger::daq_run()
{
    ...
    if (m_isDataLogging) {
        ...
        if (ret < 0) {
            std::cerr << "### CometCdcLogger: ERROR occured at data saving\n";
    ...
    if (m_debug) {
        ...
        if (seq_num % m_update_rate == 0) {
            std::cerr << "CometCdcLogger: loop = " << seq_num << std::endl;
            std::cerr << "\033[A\r";
        }
    }
...
extern "C"
{
    void CometCdcLoggerInit(RTC::Manager* manager)
    {
        RTC::Properties profile(mylogger_spec);
        manager->registerFactory(profile,
                                 RTC::Create<CometCdcLogger>,
                                 RTC::Delete<CometCdcLogger>);
    }
};
